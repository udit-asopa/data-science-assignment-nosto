...
def add_session_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if pd.api.types.is_timedelta64_dtype(out["time_spent_in_minutes"]):
        out["time_spent_in_minutes"] = (
            out["time_spent_in_minutes"].dt.total_seconds() / 60
        )

    viewed = out["total_viewed_products"]
    out["buy_ratio"] = (out["total_bought_products"] / viewed).where(
        viewed > 0, 0.0
    )
    out["cart_ratio"] = (out["total_put_in_cart_products"] / viewed).where(
        viewed > 0, 0.0
    )
...
def add_purchase_history_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["ever_bought"] = (
        past_bought.groupby(out["customer_id"]).cummax().astype(int)
    )
